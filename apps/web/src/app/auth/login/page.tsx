import { auth, signIn } from '@/lib/auth'
import { redirect } from 'next/navigation'
import Image from 'next/image'

interface Props {
  searchParams: Promise<{ callbackUrl?: string; error?: string }>
}

export default async function LoginPage({ searchParams }: Props) {
  // Check if already authenticated
  const session = await auth()
  const params = await searchParams

  // Validate callbackUrl - only allow internal paths to prevent open redirect
  const callbackUrl = params.callbackUrl && params.callbackUrl.startsWith('/')
    ? params.callbackUrl
    : '/projects'

  // If already logged in, redirect to callback
  if (session?.user) {
    redirect(callbackUrl)
  }

  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 bg-cyber-darker">
      <div className="w-full max-w-md">
        {/* Logo */}
        <div className="text-center mb-8">
          <Image
            src="/rolhack_logo.png"
            alt="RolHack"
            width={280}
            height={100}
            className="mx-auto mb-4"
            priority
          />
          <p className="text-gray-500 text-sm">
            Framework de simulacion de hacking
          </p>
        </div>

        {/* Login card */}
        <div className="bg-cyber-dark border border-cyber-primary/20 rounded-xl p-8">
          <h2 className="text-xl font-bold text-white mb-2 text-center">
            Iniciar sesion
          </h2>
          <p className="text-gray-500 text-sm text-center mb-6">
            Accede con tu cuenta de Google para continuar
          </p>

          {/* Error message */}
          {params.error && (
            <div className="bg-red-900/20 border border-red-700/30 rounded-lg p-3 mb-4">
              <p className="text-red-400 text-sm text-center">
                {params.error === 'OAuthAccountNotLinked'
                  ? 'Esta cuenta ya esta vinculada con otro metodo de inicio de sesion.'
                  : params.error === 'AccessDenied'
                  ? 'Acceso denegado. Tu cuenta puede estar desactivada.'
                  : 'Error al iniciar sesion. Intenta de nuevo.'}
              </p>
            </div>
          )}

          {/* Sign in form */}
          <form
            action={async () => {
              'use server'
              await signIn('google', { redirectTo: callbackUrl })
            }}
          >
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-800 font-medium py-3 px-4 rounded-lg transition-colors"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              Continuar con Google
            </button>
          </form>

          {/* Return link info */}
          {callbackUrl !== '/projects' && (
            <p className="text-gray-600 text-xs text-center mt-4">
              Despues del login seras redirigido automaticamente
            </p>
          )}
        </div>

        {/* Back link */}
        <div className="text-center mt-6">
          <a
            href="/"
            className="text-gray-500 hover:text-gray-400 text-sm transition-colors"
          >
            Volver al inicio
          </a>
        </div>
      </div>
    </main>
  )
}
